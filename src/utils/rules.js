// 表单校验
export const formRules = (type, maxlength, min, max = 30) => {
  const rule = {
    // 必填
    must: [
    ...
    maxLengthRequired: [
      ...
      { type: 'number', message: `正整数, 且数值大小不超过${max}`, trigger: 'blur', max },
    ...
    passwordRequired: [
      ...
      { required: true, message: '不能为空', trigger: 'blur' },
      {
        ...
        message: `密码必须为长度6-16位，且为字母、数字和非空格特殊字符的混合组合`,
        trigger: 'blur',
      },
    ],
    // 非必填
    password: { pattern: /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[~!@#$%^&*()_+`\-={}:";'<>,./]).{6,16}$/ },
  };
  return rule[type];
};
