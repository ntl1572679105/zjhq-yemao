<template>
  <PageContainer>
    <div>
      <CommonTitleAction title="可转招标需求列表">
        <template #right>
          <a-button
            type="primary"
...
import { demandPage, demandTransfer } from '@/api/operator/bid.js';
...
const transferAction = (record) => {
  if (record) {
    record.loading = true;
    demandTransfer({ idList: [record.id] })
      .then(() => {
    ...
    state.loading = true;
    demandTransfer({ idList: state.selectedRowKeys })
      .then(() => {
        proxy.$message.success('发起招标成功');
        proxy.$refs.tableRef.refresh();
