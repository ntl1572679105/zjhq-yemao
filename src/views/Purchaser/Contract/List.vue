  ...
  <PageContainer>
    <div>
      <CommonTitleAction title="合同管理">
        <template #right>
          <a-button type="primary" @click="showDetail()">上传合同</a-button>
        </template>
      </CommonTitleAction>
      <TableSearch
        ref="tableRef"
...
import { contractPage, contractCommit } from '@/api/operator/contract';
...
const showDetail = (record, type) => {
  if (record) {
    router.push({ path: `/purchasing/contract-list/0/${type}/${record.id}` });
  } else {
    router.push({ path: `/purchasing/contract-list/0/add` });
  }
};
const showApproval = (record) => {
  if (record.approvalStatus === 0) {
    router.push({ path: `/purchasing/contract-list/0/3/${record.id}` });
  } else {
    record.loading = true;
